...
def get_summaty_sys_prompt(language: str):
    return f"""
            #### BASIC INFORMATION ####
...
def get_extract_text_pdf_sys_prompt(language: str):
    return f"""
    #### BASIC INFORMATION ####
    You are an assistant reading all the text from the pdf files.

    #### TASK DESCRIPTION ####
    You get a pdf file in base64 format with text on it. Your task is to extract ALL THE MAIN CONTENT from the document and write the output.
    ...
    9. EXPLICITLY IGNORE:
        ...
        - News feeds unrelated to the main topic
    #### FORMAT REQUIREMENTS ####
        - extract ALL THE MAIN CONTENT from the picture.
        - use the exact XML tags as shown in the example below.
        - if there is no text in the picture, still use the tags but state "no text provided"
        - if there is no formula, DO NOT ADD THE FORMULA TAG.
        - if there is a formula, write it in the LATEX format.
