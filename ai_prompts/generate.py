...
from ai_prompts.gemini_prompts import *
...
async def generate_summary(request: AIRequest):
    return await gemini_request(system_prompt=get_summaty_sys_prompt(request.language), contents=request.text)
...
async def generate_test(request: AIRequest):
    return await gemini_request(system_prompt=get_test_sys_prompt(language=request.language, num_questions=request.num_questions), contents=request.text)